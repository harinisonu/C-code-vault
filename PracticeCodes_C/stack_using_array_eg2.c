...
void push(int *stack, int *count_stack) {
    ...
    if (*count_stack == MAX - 1) {
        ...
        scanf("%d", &element);
        (*count_stack)++;
        *(stack + *count_stack) = element; // Using pointer arithmetic
        printf("New element pushed in stack: %d\n", *(stack + *count_stack));
        for (int i = 0; i <= *count_stack; i++) {
            printf("Stack: %d\n", *(stack + i)); // Using pointer arithmetic
        }
    }
}
...
void pop(int *stack, int *count_stack) {
    if (*count_stack == -1) {
        ...
        printf("Element popped from stack: %d\n", *(stack + *count_stack)); // Using pointer arithmetic
        *(stack + *count_stack) = 0; // Resetting the value, not necessary but for clarity
        (*count_stack)--;
        for (int i = 0; i <= *count_stack; i++) {
            printf("Stack: %d\n", *(stack + i)); // Using pointer arithmetic
        }
